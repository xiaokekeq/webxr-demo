...
import type { DisplayMode, RegistrationStoreState } from '../../registration/registration-store.js';
...
export function createDesktopPanel(dom: ARDomElements): DesktopPanelController {
	...
	return {
		...
		render(state) {
			...
			dom.desktopPrecisionPairCountEl.textContent = `${state.precisionRegistration.pairSummaries.length} / 寤鸿鑷冲皯 4 缁刞;
...
function renderPipeList(host: HTMLElement, pipeList: PipeRecord[]): void {
	...
	const nodes = items.length === 0
		? [ createTextBlock( 'div', 'desktop-list-item', '鏆傛棤鍫ら槻瀵硅薄' ) ]
...
function createPipeListItem(item: PipeRecord): HTMLDivElement {
	...
	const detail = [ item.type, item.diameter ].filter( Boolean ).join( ' / ' ) || '鏈垎绫?;
